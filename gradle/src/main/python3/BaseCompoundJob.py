...
class BaseCompoundJob(BaseJob):
    ...
    def add_job(self, job_name, previous_job_name_list, next_job_name_list):

        new_job = WorkflowJob(self, job_name, previous_job_name_list, next_job_name_list)
        self.job_map[job_name] = new_job

        return new_job

    def add_branch_point(
            self, branch_point_name, previous_job_name_list, next_job_name_list, function, branch_job_name_list
    ):

        new_job = BranchPoint(function, previous_job_name_list, next_job_name_list, branch_job_name_list)
    ...
    def add_threaded_for_each_loop(self, loop_name, previous_job_name_list, next_job_name_list, num_threads):
        return _add_threaded_for_each_loop(self, loop_name, previous_job_name_list, next_job_name_list, num_threads)
    ...
    @staticmethod
    def run_thread(job, queue, state, workflow_data_tree, parameters, directory, iteration_row):

        try:
            new_state, new_workflow_data_tree, exception_map = job.execute(
                state, workflow_data_tree, parameters, directory, iteration_row
            )
    ...
    def start_jobs(
        ...
        while bool(job_name_set):
            ...
            if combined_trivial_type != TrivialFuture.initial:
                ...
                workflow_data = WorkflowData(*new_result_tuple[:-1], iteration_row)
                next_job_name_set = job.get_next_job_name_set()
                ready_branch_job_name_set = BaseCompoundJob.get_ready_job_name_set(
                ...
                try:
                    branch_job_name_set = job.execute(workflow_data)
                ...
                future_map[job_name] = executor.submit(
                    ...
                    job, queue, *new_result_tuple[:-1], parameters, iteration_directory, iteration_row
                )
    ...
    def execute_iteration(self, state, workflow_data_tree, parameters, directory, parent_runtime_id, iteration_num):
...
def _add_threaded_for_each_loop(self, loop_name, previous_job_name_list, next_job_name_list, num_threads):
    new_loop = ThreadedForEachLoopJob(self, loop_name, previous_job_name_list, next_job_name_list, num_threads)
    self.job_map[loop_name] = new_loop
