...
class WorkflowDataTree:
    ...
    def set_exit_status(self, exit_status):
        self.data_map[WorkflowDataTree.root_key][WorkflowDataTree.exit_status_key] = exit_status
    ...
    def get_path_data_relative(self, data_path, initial_iteration_list):
        ...
        for item in data_path:
            ...
            if child_type == WorkflowDataTree.job_type:
                return data.toList()
                # CHANGE FROM PREVIOUS SETUP -- DATA IS NO LONGER IN FORM:
                # "<activity_name> : [ <list of campaign results> ]
                # SO THE PROCESSING BEFORE "toList" BELOW IS NOT NECESSARY
    ...
    def get_value(self, data_path, function_list):
        ...
        flattened_data = [item.data for item in self.flatten_list(self.get_path_data(data_path))]
        data = self.flatten_list(flattened_data)
