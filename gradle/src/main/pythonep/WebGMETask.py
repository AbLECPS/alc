        ...
        try:
            if not self.is_complete():
                ...
                WebGMETask.logger.info(
                    "WebGMETask ({0}):  Task \"{1}\" of Job \"{2}\" trying to acquire lock ...".format(
                        class_name, self.task_name, self.job_path
                    )
                )
                with WorkflowUtils.lock:
                    WebGMETask.logger.info("WebGMETask ({0}):  Task \"{1}\" of Job \"{2}\" has acquired lock.".format(
                        class_name, self.task_name, self.job_path
                    ))
                    ...
                    while retry:
                        ...
                        try:
                            ...
                            WebGMETask.logger.info(
                                "WebGMETask ({0}):  Task \"{1}\" of Job \"{2}\" caught an exception.  "
                                "Checking if connection error.".format(class_name, self.task_name, self.job_path)
                            )
                        ...
                        if self.get_exit_status() != 0:
                            if self.check_for_connection_error():
                                if retry_count < WebGMETask._max_retry:
                                    ...
                                    retry_count += 1
                                    WebGMETask.logger.info(
                                        "WebGMETask ({0}):  Task \"{1}\" of Job \"{2}\" has detected a connection "
                                        "error, retry_count is now {3}.  Sleeping for {4} seconds ...".format(
                                            class_name,
                                            self.task_name,
                                            self.job_path,
                                            retry_count,
                                            WebGMETask._retry_sleep_interval
                                        )
                                    )
                                    ...
                                    WebGMETask.logger.info(
                                        "WebGMETask ({0}):  Task \"{1}\" of Job \"{2}\" has detected a connection "
                                        "error,  and retry_count ({3}) exceeded.  Raising exception.".format(
                                            class_name, self.task_name, self.job_path, WebGMETask._max_retry
                                        )
                                    )
                            ...
                            if not retry:
                                ...
                                raise Exception(
                                    "WebGMETask ({0}): Task \"{1}\" of job \"{2}\" has exited with a non-zero exit "
                                    "status ({3}): please check files in directory \"{4}\" to determine cause of "
                                    "failure".format(
                                        class_name,
                                        self.task_name,
                                        self.job_path,
                ...
                WebGMETask.logger.info(
                    "WebGMETask ({0}): Task \"{1}\" of Job \"{2}\" has released lock.".format(
                        class_name, self.task_name, self.job_path
                    )
                )
                ...
                try:
                    ...
                    self.write_to_failing_task_path_file()
                    raise Exception(
                        "WebGMETask ({0}): Unable to get status file of task \"{1}\" of job \"{2}\": "
                        "Exception message: \"{3}\", "
                        "please check files in directory \"{4}\" to determine cause of failure".format(
                            class_name, self.task_name, self.job_path, e, self.base_directory
                        )
                    )
