...
def max_likelihood_conditional_fit(x, y):
    ...
    p_value_sampled = np.count_nonzero(y)*1.0 / len(y)
    if (p_value_sampled > 0.99):
        p_value_sampled = 0.99
    
    #print('p_value_sampled{0}'.format(p_value_sampled))
    unconditional_likelihood = binomial_log_likelihood(p_value_sampled)

    # Print likelihood results
    #print("Estimated parameters from function-fitting (sigmoid) for event likelihood:")
    ...
    result ={}
    result["Sigmoid"] =-sig_max_likelihood_res.fun
    result["Opposite_Sigmoid"] = opposite_sig_max_likelihood
    result["Unconditional"] = unconditional_likelihood
    

    return sig_max_likelihood_res, opposite_sig_max_likelihood_coeff, result
