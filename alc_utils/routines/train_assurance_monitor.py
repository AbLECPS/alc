...
def run_assurance_monitor_training(param_dict,
                                   training_data_dirs,
                                   lec_model_dir,
                                   output_dir,
                                   am_data_formatter_path=None,
                                   lec_storage_metadata=None,
    ...
    lec_training_method = param_dict.get("lec_training_method", "rl")

    # Input checks
    if not (lec_training_method == "sl" or lec_training_method == "rl"):
        raise ValueError("AssuranceMonitor training routine received invalid LEC training method (%s). "
                         "Valid inputs are 'rl' or 'sl'." % lec_training_method)

    # FIXME: Do I need to download data here, or will that already be done?
    #       Currently assuming this is already done.
    # Find and load network metadata
    input_shape = None
    model_metadata_path = os.path.join(lec_model_dir, "model_metadata.json")
    if os.path.isfile(model_metadata_path):
        with open(model_metadata_path, 'r') as metadata_fp:
            model_metadata = json.load(metadata_fp)
            input_shape = model_metadata.get('input_shape', None)
    ...
    # TODO: For SL-trained LECs, AM should be trained on the same data. Need to recreate this data split here.

    # FIXME: Assurance Monitors also use DataFormatter, but may not have access to LEC metadata for initializing DF.
    #        How to best handle this?
    # Load and format data.
    formatter_params = {"input_shape": input_shape, "mode": "TRAINING"}
    data_formatter = alc_common.load_formatter(
        data_formatter_path, **formatter_params)
    ...
    assurance_monitor_type = param_dict.get(
        "type", assurance_monitor_type).lower()
    ...
    if lec_training_method == "rl":
        # monitor.train(training_data, validation_data, network_type='regression',
        #              calibration_output_type='predicted', **param_dict)
        if (assurance_monitor_type == 'selective_classification'):
            monitor.train(training_data, validation_data,
                          testing_data, _save_dir, **param_dict)
        else:
            monitor.train(training_data, validation_data,
                          _save_dir, **param_dict)

    if lec_training_method == "sl":
        monitor.train(training_data, validation_data, network_dir=lec_model_dir, network_type='regression',
                      calibration_output_type='labeled', **param_dict)

    # Save assurance monitor
    print("Saving trained assurance monitor...")
    save_dir = monitor.save(_save_dir, data_formatter_path=data_formatter_path,
                            lec_storage_metadata=lec_storage_metadata, make_unique_subdir=False)
    print("Saved trained assurance monitor in output directory: %s" % save_dir)

    # Upload results and return storage metadata
    uploader = alc_utils.file_uploader.FileUploader()
    return uploader.upload_with_params(output_dir, param_dict)
