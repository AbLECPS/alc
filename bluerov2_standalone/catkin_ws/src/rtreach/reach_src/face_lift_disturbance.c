...
REAL get_derivative_bounds_bicycle_disturbance(HyperRectangle* rect, int faceIndex,REAL heading_input, REAL throttle, REAL disturbance);

// Constants necessary to guarantee loop termination.
// These bound the values of the derivatives
const REAL MAX_DER_BDD = 99999;
const REAL MIN_DER_BDD = -99999;

// for benchmarking purposes having the iterations at quit is huge
int iterations_at_quitdd = 0;
...
void make_neighborhood_rect_bicycle_disturbance(HyperRectangle* out, int f, HyperRectangle* bloatedRect, HyperRectangle* originalRect, REAL nebWidth)
{
	*out = *bloatedRect;
...
REAL lift_single_rect_bicycle_disturbance(HyperRectangle* rect, REAL stepSize, REAL timeRemaining, REAL heading_input, REAL throttle, REAL disurbance)
{
	...
	while (needRecompute)
	{
		...
		for (int f = 0; f < NUM_FACES; ++f)
		{
			...
			make_neighborhood_rect_bicycle_disturbance(&faceNebRect, f, &bloatedRect, rect, nebWidth[f]);
			
			// test derivative inside neighborhood

			// the projection of the derivative on the outward norm e_i- is -fi(x) and fi(x) for e_i
			REAL der = get_derivative_bounds_bicycle_disturbance(&faceNebRect,f, heading_input, throttle,disurbance);
...
bool face_lifting_iterative_improvement_bicycle_disturbance(int startMs, LiftingSettings* settings,REAL heading_input, REAL throttle,HyperRectangle VisStates[],int  *total_intermediate,int max_intermediate,bool plot, REAL disurbance)
{
	...
	while (true)
	{
		...
		while (safe && timeRemaining > 0)
		{
			...
			REAL timeElapsed = lift_single_rect_bicycle_disturbance(&trackedRect, stepSize, timeRemaining, heading_input, throttle,disurbance);
