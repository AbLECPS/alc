...
bool face_lifting_iterative_improvement_uuv_dyn(int startMs, LiftingSettings* settings, REAL heading, REAL speed, HyperRectangle VisStates[],int  *total_intermediate,int max_intermediate,bool plot);
REAL get_derivative_bounds_uuv(HyperRectangle* rect, int faceIndex, REAL heading, REAL speed);
...
void make_neighborhood_rect_uuv_dyn(HyperRectangle* out, int f, HyperRectangle* bloatedRect, HyperRectangle* originalRect, REAL nebWidth)
{
	*out = *bloatedRect;
...
REAL lift_single_rect_uuv_dyn(HyperRectangle* rect, REAL stepSize, REAL timeRemaining, REAL heading, REAL speed)
{
	...
	while (needRecompute)
	{
		...
		for (int f = 0; f < NUM_FACES; ++f)
		{
			...
			make_neighborhood_rect_uuv_dyn(&faceNebRect, f, &bloatedRect, rect, nebWidth[f]);
			
			// test derivative inside neighborhood

			// the projection of the derivative on the outward norm e_i- is -fi(x) and fi(x) for e_i
			REAL der = get_derivative_bounds_uuv(&faceNebRect,f, heading, speed);
...
bool face_lifting_iterative_improvement_uuv_dyn(int startMs, LiftingSettings* settings,REAL heading, REAL speed,HyperRectangle VisStates[],int  *total_intermediate,int max_intermediate,bool plot)
{
	...
	while (true)
	{
		...
		while (safe && timeRemaining > 0)
		{
			...
			REAL timeElapsed = lift_single_rect_uuv_dyn(&trackedRect, stepSize, timeRemaining, heading, speed);
