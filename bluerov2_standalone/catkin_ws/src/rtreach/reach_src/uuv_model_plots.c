...
// Simulation 
REAL getSimulatedSafeTime(REAL start[4],REAL heading,REAL speed)
{
	REAL stepSize = 0.002f;
	REAL rv = 0.0f;

	simulate_uuv_plots(start, heading,speed,stepSize, shouldStop, (void*)&rv); // TODO: look here
...
{
	if (openInitial)
		f_initial = fopen("uuv_initial.gnuplot.txt", "w");

	f_intermediate = fopen("uuv_intermediate.gnuplot.txt", "w");
	f_final = fopen("uuv_final.gnuplot.txt", "w");
...
HyperRectangle runReachability_uuv_vis(REAL* start, REAL simTime, REAL wallTimeMs, REAL startMs,REAL heading, REAL speed)
{
	...
	return face_lifting_iterative_improvement_uuv_vis(startMs, &set,heading,speed,true);
}
