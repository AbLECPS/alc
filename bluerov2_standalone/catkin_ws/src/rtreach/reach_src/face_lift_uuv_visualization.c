...
HyperRectangle face_lifting_iterative_improvement_uuv_vis(int startMs, LiftingSettings* settings, REAL heading, REAL speed,bool plot);
...
void make_neighborhood_rect_uuv(HyperRectangle* out, int f, HyperRectangle* bloatedRect, HyperRectangle* originalRect, REAL nebWidth)
{
	*out = *bloatedRect;
...
REAL lift_single_rect_uuv(HyperRectangle* rect, REAL stepSize, REAL timeRemaining, REAL heading, REAL speed)
{
	...
	while (needRecompute)
	{
		...
		for (int f = 0; f < NUM_FACES; ++f)
		{
			...
			make_neighborhood_rect_uuv(&faceNebRect, f, &bloatedRect, rect, nebWidth[f]);
...
HyperRectangle face_lifting_iterative_improvement_uuv_vis(int startMs, LiftingSettings* settings,REAL heading, REAL speed,bool plot)
{
	...
	while (true)
	{
		...
		while (timeRemaining > 0)
		{
			...
			REAL timeElapsed = lift_single_rect_uuv(&trackedRect, stepSize, timeRemaining, heading, speed);
