...
bool check_safety(HyperRectangle* rect, REAL (*cone)[2])
{
	...
	rect->dims[0].min = rect->dims[0].min  - 0.2285;
    rect->dims[0].max = rect->dims[0].max  + 0.2285;
    rect->dims[1].min = rect->dims[1].min  - 0.169;
    rect->dims[1].max = rect->dims[1].max  + 0.169;
