  ...
  if(count>0)
  {
    ...
    if(true)
    {
      ...
      for(int i = 0; i<num_intermediate;i++)
      {
        ...
        myQuaternion.setRPY(0.0,0.0,(hull.dims[2].max+hull.dims[2].min)/2.0);
...
int main(int argc, char **argv)
{
    ...
    vis_pub = n.advertise<visualization_msgs::MarkerArray>( "reach_hull2", 100 );
  
    sub = n.subscribe("uuv0/hsd_command", 1000, hsd_callback);
    sub2 = n.subscribe("uuv0/pose_gt", 1000, callback);
