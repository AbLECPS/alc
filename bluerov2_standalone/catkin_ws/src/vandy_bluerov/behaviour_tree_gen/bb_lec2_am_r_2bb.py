...
class ToBlackboard(subscribers.ToBlackboard):
    ...
    def __init__(self, 
        ...
        self.blackboard.lec2_am_r = Float32MultiArray()
...
############<<USER UPDATE CODE BEGINS>>##############################
        log_m = self.blackboard.lec2_am_r.data[-2]
        if log_m >= self.speed_good_log_val:
            self.blackboard.lec2_am_r_speed_warning = True
            rospy.logwarn_throttle(1, "%s: lec2_am_rs is high!" % self.name)
        elif log_m < self.speed_good_log_val:
            self.blackboard.lec2_am_r_speed_warning = False
        # else don't do anything in between - i.e. avoid the ping pong problems

        if log_m >= self.pipe_estimation_good_log_val :
            self.blackboard.lec2_am_r_pipe_warning = True
            rospy.logwarn_throttle(1, "%s: lec2_am_rp is high!" % self.name)
