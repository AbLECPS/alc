csvfilename = 'Exp14/data/data_sys_id1.csv';
data = load(csvfilename);
data(:,3) = convert_yaw(data);
x = data(1:end-1,[1,2,3]);
u = data(1:end-1,[5,6]);
...
for j=1:length(u)
    ...
    for i=1:length(simrange)
        xe{i+1} = simulate_continuous_model(xe{i},u(j,:),step_size);
    end
    xj{j,1} = xe; 
end
...
figname = split(strrep(csvfilename,'Exp14/data/',''),".");
savename = strcat("val_images/",'sim',figname{1},".png");
saveas(fig,savename);
