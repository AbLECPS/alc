...
class ThrusterManager:
    ...
    def __init__(self):
        ...
        rospy.loginfo('inverse_configuration_matrix=')
    ...
    def reallocate_tam(self, tam):
        """Realocate the thruster allocation matrix, when thruster degradaton detected."""
        
        self.ready = False
        rospy.loginfo('ThrusterManager: updating TAM')
        
        if len(tam) == self.n_thrusters * 6:
            self.configuration_matrix = numpy.reshape(tam,(6,6))
            # for i in range(self.n_thrusters):
            #     self.thrusters[i].tam_column = self.configuration_matrix[:, i]
            rospy.loginfo('Thruster allocation matrix provided!')
            rospy.loginfo('TAM=')
            rospy.loginfo(self.configuration_matrix)
            self.inverse_configuration_matrix = numpy.linalg.pinv(
