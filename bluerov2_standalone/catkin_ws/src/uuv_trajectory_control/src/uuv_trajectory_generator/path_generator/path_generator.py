...
class PathGenerator(object):
    ...
    def __init__(self, full_dof=False):
        ...
        # There's a 5 second delay between marker and AUV robot
        self.delay = 5
    ...
    @property
    def most_recent_waypoint_idx(self):
        """
        Return the index of the most recently passed waypoint.
        """
        if self._cur_s == 0:
            return 0
        elif self._cur_s == 1:
            return len(self._s) - 1
        v = -1 * np.array(self._s - self._cur_s + (self.delay/self.max_time))
        v[v<0] = 1.1
        idx = np.argmin(v)
        return idx

    @property
    def most_recent_waypoint(self):
        return self._waypoints.get_waypoint(self.most_recent_waypoint_idx)
