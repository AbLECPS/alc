...
class DPControllerLocalPlanner(object):
    ...
    def interpolate(self, t):
        ...
        if not self._station_keeping_on and self._traj_running:
            ...
            #######################################################################
        ######################################################################
        # Edited Section: Checks the last waypoint to see which controller should be used
        ######################################################################
        #controllerID = self._traj_interpolator.get_vehicle_controller(t)
        self._lock.release()
        return self._this_ref_pnt#, controllerID
