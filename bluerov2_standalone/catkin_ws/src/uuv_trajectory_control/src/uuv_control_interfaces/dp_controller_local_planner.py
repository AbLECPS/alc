...
from vandy_bluerov.srv import GoToHSD, GoToHSDResponse
...
class DPControllerLocalPlanner(object):
    ...
    def __init__(self, full_dof=False, stamped_pose_only=False, thrusters_only=True):
        ...
        # self._services['stop_vehicle'] = rospy.Service(
        #     'stop_vehicle', Hold, self.stop_vehicle)
        ...
        self._services['go_to_HSD'] = rospy.Service('go_to_HSD', GoToHSD, self.go_to_HSD)
    ...
    # def stop_vehicle(self, request):
    #     self._lock.acquire()
    #     # for serv in self._services:
    #     #     self._services[serv].shutdown()
    #     # self.__init__()
    #     # self._lock.release()
    #     return HoldResponse(True)
    ...
    def go_to(self, request):
        time.sleep(3.0) #wait for current pose
        if self._vehicle_pose is None:
    ...
    def go_to_HSD(self, request):
        while self._vehicle_pose is None:
             time.sleep(1) #wait for current pose
             self._logger.error('Waiting for current pose')
        if self._vehicle_pose is None:
            self._logger.error('Current pose has not been initialized yet')
            return GoToHSDResponse(False)
        ...
        init_wp = uuv_waypoints.Waypoint(
            ...
            y=self._vehicle_pose.pos[1],
            z=self._vehicle_pose.pos[2],
            max_forward_speed=request.speed,
            heading_offset=euler_from_quaternion(self.get_vehicle_rot())[2],
            use_fixed_heading=False,
            inertial_frame_id=self.inertial_frame_id)
        wp_set.add_waypoint(init_wp)
        new_wp = uuv_waypoints.Waypoint(
            x=self._vehicle_pose.pos[0]+50*math.cos((request.heading)), #50m distance
            y=self._vehicle_pose.pos[1]+50*math.sin((request.heading)), #50m distance
            z=request.depth,
            max_forward_speed=request.speed,
            heading_offset=0,
            use_fixed_heading=False,
        ...
        self._logger.info('============================')
        self._logger.info('GO TO HSD')
        self._logger.info('============================')
        self._logger.info('# heading = %0.2f rad' % request.heading)
        self._logger.info('# speed = %0.2f m/s' % request.speed)
        self._logger.info('# depth = %0.2fm' % request.depth)
        self._logger.info('Starting from = ' + str(self._traj_interpolator.get_waypoints().get_waypoint(0).pos))
        self._logger.info('Start time [s] = %.2f ' % t)
        self._logger.info('Inertial frame ID = ' + self.inertial_frame_id)
        self._logger.info('============================')
