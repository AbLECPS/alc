"""
This script is called by the plugin-wrapper, IDELaunch.js, which passes down the
...
from IDELaunch import IDELaunch
...
logger.debug('userid: {0}'.format(USERID))

# Create an instance of WebGME and the plugin
webgme = WebGME(PORT, logger)
plugin = IDELaunch(webgme, COMMIT_HASH, BRANCH_NAME, ACTIVE_NODE_PATH, ACTIVE_SELECTION_PATHS, NAMESPACE,USERID)
