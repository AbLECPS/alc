...
define([
        ...
        core.loadSubTree(model)
            .then(function (nodes) {
                ...
                nodes.forEach(function (node) {
					...
					if (core.isInstanceOf(node, self.META.PlantModel)) {
						plantModel = node;						
					}
                });

                core.getChildrenPaths(plantModel).forEach(function (childPath) {
                    if (core.isInstanceOf(nodeCache[childPath], self.META["SpaceEx.BaseComponent"])) {
                        self._saveBaseComponent(modelObject, nodeCache, nodeCache[childPath]);
                    }

                    if (core.isInstanceOf(nodeCache[childPath], self.META["SpaceEx.NetworkComponent"])) {
                ...
                self._result = {};
                self._result.filename = core.getAttribute(plantModel, 'name') + '.xml';
    ...
    ExportSpaceEx.prototype._saveMapping = function (serializedContainerObject, nodeCache, node, bindHash) {
        ...
        if (core.isTypeOf(dst, self.META["SpaceEx.Parameter"])) {
            HyST.setContent(mapping, core.getAttribute(dst, 'name'));
        } else if (core.isTypeOf(dst, self.META["SpaceEx.Constant"])) {
    ...
    ExportSpaceEx.prototype._saveBaseComponent = function (serializedContainerObject, nodeCache, node) {
        ...
        core.getChildrenPaths(node).forEach(function (childPath) {
            if (core.isInstanceOf(nodeCache[childPath], self.META["SpaceEx.Parameter"])) {
                self._saveParameter(component, nodeCache, nodeCache[childPath]);
            }
        });
        core.getChildrenPaths(node).forEach(function (childPath) {
            if (core.isInstanceOf(nodeCache[childPath], self.META["SpaceEx.Location"])) {
                self._saveLocation(component, nodeCache, nodeCache[childPath]);
            }
        });
        core.getChildrenPaths(node).forEach(function (childPath) {
            if (core.isInstanceOf(nodeCache[childPath], self.META["SpaceEx.Transition"])) {
    ...
    ExportSpaceEx.prototype._saveNetworkComponent = function (serializedContainerObject, nodeCache, node) {
        ...
        core.getChildrenPaths(node).forEach(function (childPath) {
            if (core.isInstanceOf(nodeCache[childPath], self.META["SpaceEx.Component"])) {
                self._saveBinding(component, nodeCache, nodeCache[childPath], bindHash);
            }
        });

        core.getChildrenPaths(node).forEach(function (childPath) {
            if (core.isInstanceOf(nodeCache[childPath], self.META["SpaceEx.Mapping"])) {
