...
class VerificationRobustnessRunner:
    ...
    def check_mean_std(eng, image, mean, std):
        ...
        return_mean = VerificationRobustnessRunner.check_value(num_channels, mean)
        if return_mean is None:
            raise RuntimeError("Please specify a valid mean")
        return_mean = mean

        return_std = VerificationRobustnessRunner.check_value(num_channels, std)
...
if __name__ == '__main__':
    ...
    path = Path(json_file).absolute()
    
    launch_verification_interpreter = VerificationRobustnessRunner(path.parent)
