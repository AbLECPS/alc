...
if (len(sys.argv) < 2 ):
    usage_str ='''
usage: run_assurance <PROJECT_NAME>  [OWNER_NAME] [-r]
option:
   OWNER_NAME : specify owner name for the project. (default = admin)
   -r         : prints reason during execution (default = False)
'''
    print (usage_str)
    exit(1)
...
node_process = subprocess.Popen(['node', COREZMQ_SERVER_FILE, PROJECT_NAME, '-p', PORT, '-m', METADATA_PATH, '-o', OWNER_NAME, '-u', 'admin:vanderbilt'],
