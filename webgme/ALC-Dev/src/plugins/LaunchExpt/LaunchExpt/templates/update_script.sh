...
update_webgme_internal() {
  ...
  node ./node_modules/webgme-engine/src/bin/run_plugin.js ALCModelUpdater {{ project_name|string }} \
    ...
    -j $ALC_WORKING_DIR/{{ relative_update_model_config_path|string }} \
    -n ALCMeta \
...
UPDATE_SCRIPT_LOG={{ result_dir|string }}/update_script_log.txt
...
if [ -z $1 ]; then
  ...
  echo "Finished_w_Errors" > {{ result_dir|string }}/slurm_exec_status.txt
  update_webgme
  exit 1
fi
...
if [[ "$JOB_STATUS" == "RUNNING" ]]; then
  ...
  echo "Started" > {{ result_dir|string }}/slurm_exec_status.txt
  update_webgme
...
if [[ "$JOB_STATUS" == "COMPLETED" ]]; then
  ...
  echo "Finished" > {{ result_dir|string }}/slurm_exec_status.txt
else
...
{% if (wf_output_path) %}
# Copy final job status file to specified location for workflow engine
check_dir="$(dirname {{ result_dir|string }})"
finished_status_count=($(find "$check_dir" -name slurm_exec_status.txt -exec grep Finished {} + | wc -l))
if [ "$finished_status_count" -eq {{ camp_count }} ]; then
  error_status_count=($(find "$check_dir" -name slurm_exec_status.txt -exec grep Finished_w_Errors {} + | wc -l))
  if [ "$error_status_count" -gt 0 ]; then
    echo "Finished_w_Errors" > {{ wf_output_path|string }}
  else
    echo "Finished" > {{ wf_output_path|string }}
  fi
fi
{% endif %}
